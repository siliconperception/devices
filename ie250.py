# This file produces reference PyTorch models for the IE250 image encoder
...
parser.add_argument('--model', help='model name',default='ie250-050-500')
...
parser.add_argument('--batch', help='unit test batch size',default=3, type=int)
parser.add_argument('--lr', help='unit test learning rate',default=0.0001, type=float)
parser.add_argument('--show', help='display batches during unit test',default=False, action='store_true')
parser.add_argument('--seed', help='random seed',default=None, type=int)
args = parser.parse_args()
args.rs = np.random.RandomState(np.random.MT19937(np.random.SeedSequence(args.seed)))
print(args)

class IE250(nn.Module):
    def __init__(self, model):
        super(IE250, self).__init__()
        if model=='ie250-050-500':
            self.width = 700
            self.height = 700
            self.fm_width = 2
            self.fm_height = 2
        ...
        if self.extra:
            self.layer19 = nn.Sequential(nn.Conv2d(512, 512, kernel_size=3, stride=2), nn.BatchNorm2d(512), nn.ReLU())
            self.layer20 = nn.Sequential(nn.Conv2d(512, 512, kernel_size=3, stride=1), nn.BatchNorm2d(512), nn.ReLU())
            self.layer21 = nn.Sequential(nn.Conv2d(512, 512, kernel_size=3, stride=1), nn.BatchNorm2d(512), nn.ReLU())
    ...
    def save(self, model):
        ...
        if self.extra:
            enc.extend([
            ('conv19', self.fuse_conv_and_bn(self.layer19[0],self.layer19[1])), ('relu19', nn.ReLU()),
            ('conv20', self.fuse_conv_and_bn(self.layer20[0],self.layer20[1])), ('relu20', nn.ReLU()),
            ('conv21', self.fuse_conv_and_bn(self.layer21[0],self.layer21[1])), ('relu21', nn.ReLU()),
            ])
        enc = nn.Sequential(collections.OrderedDict(enc))
...
def generate_batch(args,width,height,lvec,spacex,spacey):
    ...
    if args.show:
        ...
        while dw>1000:
            dw = dw//2
            dh = dh//2
        cv2.imshow('ie250', cv2.resize(d[0],dsize=(dw,dh),interpolation=cv2.INTER_LINEAR)) 
        cv2.waitKey(1)
